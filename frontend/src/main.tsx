...
import { analyzeMatch, analyzeMatchId, analyzeTeam, createMatchSummary, getPlayer } from "./lib/api";
...
function App() {
  ...
  async function generateSummary() {
    ...
    setError("");
    try {
      const response = await createMatchSummary({
        match_id: matchMeta?.id,
        team_names: matchMeta?.teamNames ?? [],
        analysis: match,
      });
      setSummary(response.summary);
      setSummaryModel(response.generated_by);
    } catch (err) {
...
function SummaryPanel(props: { summary: string; model: string; loading: boolean; onGenerate: () => void }) {
  return (
    <section className="rounded border border-line bg-panel p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-semibold"><Sparkles size={19} /> AI Match Summary</h2>
          <p className="mt-1 text-sm text-zinc-500">Optional AI write-up based only on the calculated FACEIT stats above.</p>
        </div>
        <button className="primary-button" onClick={props.onGenerate} disabled={props.loading}>
          {props.loading ? <RefreshCw className="animate-spin" size={16} /> : <Sparkles size={16} />}
          {props.loading ? "Writing" : "Generate Summary"}
        </button>
      </div>
      {props.summary ? (
        <div className="mt-4 rounded border border-line bg-ink/70 p-4">
          <p className="whitespace-pre-wrap text-sm leading-7 text-zinc-200">{props.summary}</p>
          <p className="mt-3 text-xs uppercase tracking-[0.16em] text-zinc-600">Generated by {props.model}</p>
        </div>
      ) : null}
    </section>
  );
